...
export default function AdminChangeRequestsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-change-requests', page, departmentId],
    queryFn: async () => {
      const res = await fetch(`/api/v1/admin/change-requests?${queryParams.toString()}`);
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/v1/admin/change-requests/${id}/approve`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminResponse: 'Approved' }),
      });
      const json = await res.json();
      if (!json.success) throw new Error(json.error?.message);
    },
  ...
  const rejectMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await fetch(`/api/v1/admin/change-requests/${id}/reject`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminResponse: 'Rejected' }),
      });
      const json = await res.json();
      if (!json.success) throw new Error(json.error?.message);
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Review and approve/reject employee change requests</p>
      </div>
      ...
      {isLoading && (
        <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-28 bg-muted animate-pulse rounded-md" />)}</div>
      )}
      ...
      {changeRequests.map((cr: Record<string, unknown>) => {
        ...
        return (
          <Card key={cr.id as string} className="border-orange-200">
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{crUser.name}</span>
                    <span className="text-xs text-muted-foreground">({crUser.employeeId})</span>
                    ...
                    {['CANCEL_BOOKING', 'CANCEL_PICKUP', 'CANCEL_DROP'].includes(cr.requestedField as string) ? (
                      <span className="text-destructive font-medium">
                        {cr.requestedField === 'CANCEL_BOOKING' ? 'Cancel entire booking' :
                         cr.requestedField === 'CANCEL_PICKUP' ? 'Cancel pickup only' : 'Cancel drop only'}
                      </span>
                    ) : (
                      <>
                        <span className="text-muted-foreground">Change </span>
                        <span className="font-medium">{(cr.requestedField as string).replace(/_/g, ' ').toLowerCase()}</span>
                        <br /><span className="text-muted-foreground">From: </span><span>{originalValue}</span>
                        <br /><span className="text-muted-foreground">To: </span><span className="font-medium">{cr.requestedValue as string}</span>
                      </>
                    )}
                  </div>
                ...
                <div className="flex gap-2 shrink-0">
                  <Button size="sm" onClick={() => approveMutation.mutate(cr.id as string)} disabled={approveMutation.isPending || rejectMutation.isPending}>
                    <CheckCircle className="h-4 w-4 mr-1" /> Approve
                  </Button>
                  <Button size="sm" variant="outline" className="text-destructive" onClick={() => rejectMutation.mutate(cr.id as string)} disabled={approveMutation.isPending || rejectMutation.isPending}>
