...
export default function SessionsPage() {
  ...
  const revokeSession = useMutation({
    mutationFn: async (sessionId: string) => {
      const res = await fetch(`/api/v1/super-admin/sessions?id=${sessionId}`, { method: 'DELETE' });
  ...
  const revokeAllForUser = useMutation({
    mutationFn: async (userId: string) => {
      const res = await fetch(`/api/v1/super-admin/sessions?userId=${userId}`, { method: 'DELETE' });
      const json = await res.json();
      if (!json.success) throw new Error(json.error?.message);
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['sa-sessions'] }),
  ...
  const groupedByUser: Record<string, { user: { name: string; employeeId: string }; sessions: Array<Record<string, unknown>> }> = {};
  for (const session of (sessions || [])) {
    const userId = session.userId as string;
    if (!groupedByUser[userId]) {
      groupedByUser[userId] = { user: session.user, sessions: [] };
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">View active sessions and revoke access. Sessions persist until revoked here.</p>
      </div>

      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Shield className="h-4 w-4" />
        <span>{(sessions || []).length} active session(s) across {Object.keys(groupedByUser).length} user(s)</span>
      </div>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <Badge className="ml-2 bg-blue-100 text-blue-800">{userSessions.length} session(s)</Badge>
                  </div>
                  <Button
                    variant="destructive" size="sm"
                    onClick={() => revokeAllForUser.mutate(userId)}
                    ...
                    <div key={s.id as string} className="flex items-center justify-between bg-muted/50 rounded px-3 py-2 text-sm">
                      <div>
                        <span className="font-mono text-xs text-muted-foreground">
                          {(s.token as string).slice(0, 8)}...{(s.token as string).slice(-8)}
                        </span>
                        <span className="text-xs text-muted-foreground ml-3">
                          Created: {new Date(s.createdAt as string).toLocaleString()}
                        </span>
                      </div>
                      <Button
